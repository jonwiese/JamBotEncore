...
def over_sample(mid):
    p_double = mid.get_piano_roll(fs=settings.sampling_frequency * settings.over_sample_factor)
    p = []
    for i in range(0, p_double.shape[1], settings.over_sample_factor):
        vec = np.sum(p_double[:, i:(i + settings.over_sample_factor)], axis=1)
        #        print(vec[36])
...
def pianoroll_to_midi(pianoroll, midi_folder, filename, instrument_name, bpm):
    ...
    for i in range(ind[0].shape[0]):
        note = pm.Note(velocity=80, pitch=ind[1][i], start=(60 / (2 * bpm)) * ind[0][i],
    ...
    for i, note_vector in enumerate(pianoroll):
        ...
        for note in tracker:
            if note in notes and (i) % 8 is not 0:
                ...
                midi_note = pm.Note(velocity=80, pitch=note, start=(60 / (2 * bpm)) * start_times[note],
                                    end=(60 / (2 * bpm)) * i)
